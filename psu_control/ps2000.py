...
# Imported from:
# https://github.com/marcj71/ps2000/tree/master
...
class ps2000(object):
	...
	def __init__(self, port='/dev/ttyACM0', *, logger):
	...
	def _check_checksum(self, ans):
		...
		if (ans[-2] != (cs >> 8)) or (ans[-1] != (cs & 0xff)):
			self.logger.error('error in PSU control: checksum mismatch')
			raise ValueError(
				f"Error in PSU control - checksum mismatch. Received {ans}"
			)
	...
	def _check_error(self, ans):
		...
		# TODO
		# make this logic prettier/simpler
		if ans[2] != 0xff:
			return False
		...
		if ans[3] == 0x00:
			...
			err = 'ERROR: checksum incorrect'
		elif ans[3] == 0x04:
			err = 'ERROR: start delimiter incorrect'
		elif ans[3] == 0x05:
			err = 'ERROR: wrong address for output'
		elif ans[3] == 0x07:
			err = 'ERROR: object not defined'
		elif ans[3] == 0x08:
			err = 'ERROR: object length incorrect'
		elif ans[3] == 0x09:
			err = 'ERROR: access denied'
		elif ans[3] == 0x0f:
			err = 'ERROR: device is locked'
		elif ans[3] == 0x30:
			err = 'ERROR: upper limit exceeded'
		elif ans[3] == 0x31:
			err = 'ERROR: lower limit exceeded'

		s: str = 'answer: '
		for b in ans:
			s += f"{b:02x} "
		self.logger.error(f"{err}. Received: {s}")
	...
	def get_current_stats(self, *lin_args, **kwargs):
		stats = self.get_actual(*lin_args, **kwargs)
		return PowerStats.from_dict(stats)
...
def print_info(ps):
	...
	print('control	  0x%04x' % ps.get_remote())
	print('output	   0x%04x' % ps.get_output_on())
	ps.get_actual(True)
